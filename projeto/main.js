...
// 1. Importamos as classes que você criou (ajuste o caminho se necessário)
...
prof1.setDiscipline('Banco de Dados SQL');
...
prof2.setDiscipline('Programação Node.js');
...
instituicao.forEach((cadastro, index) => {
  ...
  console.log(
    `- E-mail: ${
      cadastro.objeto.getEmail() || '❌ FALHA NA VALIDAÇÃO (E-mail Vazio)'
    }`
  );
  ...
  if (cadastro.tipo === 'Aluno') {
    console.log(
      `- Matrícula: ${
        ...
        '❌ FALHA NA VALIDAÇÃO (Matrícula Vazia)'
      }`
    );
  }
